...
- `pkg/gateway` — gRPC and grpc-gateway listener primitives:
  ...
  - Optional chain-ordered slices on `Params` / `GrpcParams`: `UnaryInterceptors`, `StreamInterceptors`, `HTTPMiddleware`. Consumer provides via plain `fx.Provide` — no opinionated defaults.
